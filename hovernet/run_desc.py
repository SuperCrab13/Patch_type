...
# from scipy.misc.utils import center_pad_to_shape, cropping_center
...
def infer_step(batch_data, model):
    ...
    # --------------------------------------------------------------
    with torch.no_grad():  # dont compute gradient
        pred_dict, features = model(patch_imgs_gpu)
        pred_dict = OrderedDict(
            [[k, v.permute(0, 2, 3, 1).contiguous()] for k, v in pred_dict.items()]
        )
    #     pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)[..., 1:]
    ...
    # return pred_output.cpu().numpy()
        type_map = F.softmax(pred_dict["tp"], dim=-1)
        type_map = torch.argmax(type_map, dim=-1, keepdim=True)
        type_map = type_map.type(torch.int64)
    
    return type_map.cpu().numpy(), features.cpu().numpy()
